...
This is an operator/development tool, not an official evaluator. BTC has not
provided official KIS queries/ground truth in the repository context, so the
command accepts a natural-language query and emits ranked candidates for
manual inspection.
...
The data package itself is built with ``tools/build_data_package.py`` from the
BTC-provided local data tree.
"""
...
def _make_html(
    ...
    for candidate in candidates[:max_cards]:
        ...
        cards.append(
            "<article class='card'>"
            f"<div class='rank'>#{candidate.get('rank', '?')}</div>"
            f"{image_html}"
            "<div class='meta'>"
            f"<b>{html.escape(video_id)}</b> · frame {frame_id}<br>"
            f"final={float(candidate.get('score', 0.0)):.6f} · "
            f"retrieval={float(candidate.get('retrieval_score') or 0.0):.6f} · "
...
<html><head><meta charset="utf-8"><title>AIC KIS local inspection</title>
<style>
body{font-family:system-ui,sans-serif;margin:24px;background:#f5f5f5;color:#222}
h1{font-size:22px}.query{padding:12px;background:white;border-radius:8px;margin-bottom:18px}
.grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(280px,1fr));gap:14px}
.card{background:white;border-radius:10px;overflow:hidden;box-shadow:0 1px 4px #bbb;position:relative}
.card img{display:block;width:100%;aspect-ratio:16/9;object-fit:cover;background:#ddd}
.rank{position:absolute;z-index:2;top:8px;left:8px;background:#111;color:white;padding:4px 8px;border-radius:6px;font-weight:700}
.meta{padding:10px;font-size:13px;line-height:1.55}.missing{height:158px;display:grid;place-items:center;background:#ddd;color:#666}
</style></head><body>
<h1>AIC 2026 — local KIS inspection</h1>
<div class="query"><b>Query:</b> __QUERY__<br><b>Candidates:</b> __COUNT__</div>
...
def main() -> int:
    ...
    parser.add_argument("query", help="Natural-language KIS query")
    ...
    parser.add_argument("--json-out", type=Path, help="Write ranked KIS candidates as JSON")
    parser.add_argument("--html-out", type=Path, help="Write a visual inspection HTML page")
    ...
    semantic_config = SemanticRerankConfig(
        enabled=not args.no_semantic,
        model_id=args.semantic_model,
        candidate_limit=args.semantic_candidates,
        weight=args.semantic_weight,
    )
    semantic_scorer = None if args.no_semantic else SigLIP2ImageTextScorer(
        model_id=args.semantic_model,
        device=args.device,
    )
    ...
    for candidate in response.candidates:
        evidence = candidate.get("evidence") or {}
        print(
            f"rank={int(candidate['rank']):03d} "
            f"score={float(candidate['score']):.6f} "
            f"video={candidate['video_id']} "
            f"frame={candidate['frame_id']} "
            f"retrieval={float(candidate.get('retrieval_score') or 0.0):.6f} "
            f"temporal={float(candidate.get('temporal_score') or 0.0):.6f} "
            f"semantic={float(evidence.get('semantic_score') or 0.0):.6f}"
        )
