...
def _media_info(payload: Any) -> dict[str, Any]:
    ...
    total_frames = _int(
        video_stream.get(
            ...
            video_stream.get(
                "nb_read_frames",
                payload.get("total_frames", 0) if isinstance(payload, dict) else 0,
            ),
        )
    )
    return {
...
def _find_artifact_file(directory: Path, n: str, frame_idx: str, suffix: str) -> Path | None:
    ...
    target_values = n_values | frame_values
    for path in files:
        stem = path.stem
        if stem.isdigit() and str(int(stem)) in target_values:
            return path
        tokens = {str(int(x)) for x in re.findall(r"\d+", stem)}
        if tokens & target_values:
            return path
    return None
        ...
        for video_id in ids:
            ...
            conn.execute(
                ...
                (video_id, stored_video_path, media["fps"], media["width"], media["height"],
                 media["duration"], media["total_frames"], None, int(video_id in media_files)),
            )
            if media_payload:
                conn.execute(
            ...
            for row in rows:
                ...
                frame_path = _find_artifact_file(frame_dir, row["n"], row["frame_idx"], "jpg")
                ...
                if object_dir.is_dir():
                    exact = object_dir / f"{frame_path.stem}.json"
                    object_path = exact if exact.is_file() else _find_artifact_file(
                        object_dir, row["n"], row["frame_idx"], "json"
                    )
